--[[
    Hallwayz Fly — Dynamic Edition (Premium Vector)
...
-- ============================================================
-- ICONS — LOGO PAKAI rbxassetid (ISI SETELAH UPLOAD)
-- ============================================================
-- Contoh: Logo = "rbxassetid://1234567890"
-- Semua icon lain otomatis pakai PREMIUM VECTOR (tidak perlu diisi)
local Icons = {
    Logo = "rbxassetid://96960479150798", -- ISI DI SINI SETELAH UPLOAD LOGO KE ROBLOX
}

-- ============================================================
-- THEME MODULE (13 WARNA)
-- ============================================================
local Themes = {
    ["Black Metal"] = {
        name = "Black Metal",
...
-- ============================================================
local ThemeRegistry = {
    frames = {}, strokes = {}, texts = {}, accents = {},
    accentsDark = {}, accentsGlow = {}, borders = {}, cards = {},
    sidebars = {}, dividers = {}, textMuted = {},
}
...
-- ============================================================
-- PREMIUM VECTOR ICON LIBRARY
-- Semua icon digambar pakai Frame + UICorner (rounded) + rotation
-- ============================================================
local VectorIcons = {}
...
function VectorIcons.Info(parent, color)
    ...
    -- outer ring (ring dibentuk pakai 2 lingkaran)
    local outer = rframe(h, UDim2.new(1, 0, 1, 0), UDim2.new(0, 0, 0, 0), color, 100)
    local inner = rframe(h, UDim2.new(1, -4, 1, -4), UDim2.new(0, 2, 0, 2), Theme.Card, 100)
    -- dot atas
    rframe(h, UDim2.new(0, 2, 0, 2), UDim2.new(0.5, -1, 0, 3), color, 1)
    -- stem bawah (rounded bar)
    rframe(h, UDim2.new(0, 2, 0, 6), UDim2.new(0.5, -1, 0, 7), color, 1)
    return h
end

-- FLY: wing berbentuk lengkung 3 layer (premium)
...
-- PALETTE: 4 color dots premium + ring
function VectorIcons.Palette(parent, color)
    local h = new("Frame", { Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, Parent = parent })
    -- ring luar
    local ring = rframe(h, UDim2.new(1, 0, 1, 0), UDim2.new(0, 0, 0, 0), color, 100)
    local ringInner = rframe(h, UDim2.new(1, -3, 1, -3), UDim2.new(0, 1.5, 0, 1.5), Theme.Card, 100)
    -- 4 dot warna
...
-- SPEED: gauge premium (3 line bertingkat + needle)
...
-- KEY: premium (head rounded + shaft + teeth)
function VectorIcons.Key(parent, color)
    local h = new("Frame", { Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, Parent = parent })
    -- head
    local head = rframe(h, UDim2.new(0, 9, 0, 9), UDim2.new(0, 0, 0, 3), color, 100)
    local hole = rframe(head, UDim2.new(0, 3, 0, 3), UDim2.new(0.5, -1.5, 0.5, -1.5), Theme.Card, 100)
    -- shaft
...
-- MINIMIZE: line premium (thick + rounded)
...
-- Buat icon: kalau Icons[kind] ada & pakai rbxassetid → pakai itu
-- Kalau tidak → pakai vector premium
...
-- Logo: kalau Icons.Logo diisi pakai rbxassetid → pakai itu
-- Kalau kosong → logo vector Hallwayz (parallelogram)
local function buildLogo(parent, size)
    ...
    -- Fallback: logo Hallwayz vector
    local sk = size / 32
    reg(ThemeRegistry.accents, rframe(holder, UDim2.new(0, 8*sk, 0, 13*sk), UDim2.new(0, 6*sk, 0, 3*sk),   Theme.Accent, 1, 20))
    reg(ThemeRegistry.accents, rframe(holder, UDim2.new(0, 8*sk, 0, 13*sk), UDim2.new(0, 6*sk, 0, 16*sk),  Theme.Accent, 1, 20))
    reg(ThemeRegistry.accents, rframe(holder, UDim2.new(0, 9*sk, 0, 26*sk), UDim2.new(0, 17*sk, 0, 3*sk),  Theme.Accent, 1, 20))
    reg(ThemeRegistry.accentsGlow or ThemeRegistry.accents,
...
local function buildKeyUI(onSuccess)
    ...
    corner(14, KeyCard)
    reg(ThemeRegistry.strokes, stroke(Theme.Border, 1, KeyCard))
    ...
    corner(8, InputBox)
    reg(ThemeRegistry.strokes, stroke(Theme.Border, 1, InputBox))
...
local function buildMainUI()
    ...
    corner(14, Main)
    reg(ThemeRegistry.strokes, stroke(Theme.Border, 1, Main))
    ...
    toggleBg.MouseButton1Click:Connect(function()
        flyOn = not flyOn
        updateToggle()
        if flyOn then NOFLY(); task.wait(0.05); sFLY() else NOFLY() end
    end)
    ...
    for _, preset in ipairs({1, 10, 50, 100, 500, 1000}) do
        ...
        corner(8, pb)
        reg(ThemeRegistry.strokes, stroke(Theme.Border, 1, pb))
