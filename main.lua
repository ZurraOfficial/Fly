...
local function grad(p, c1, c2, rot)
    return new("UIGradient", { Color = ColorSequence.new(c1, c2), Rotation = rot or 90, Parent = p })
end
...
local function applyTheme(name)
    ...
    for _, e in ipairs(Tracked.bg) do
        if e.obj and e.obj.Parent then
            pcall(function() e.obj.BackgroundColor3 = Theme[e.key] end)
        end
    end
    for _, e in ipairs(Tracked.text) do
        if e.obj and e.obj.Parent then
            pcall(function() e.obj.TextColor3 = Theme[e.key] end)
        end
    end
    for _, e in ipairs(Tracked.stroke) do
        if e.obj and e.obj.Parent then
            pcall(function() e.obj.Color = Theme[e.key] end)
        end
    end
end
...
function VI.Speed(p, c)
    ...
    rf(h, UDim2.new(0,5,0,1), UDim2.new(1,-6,0,8), c, 1, -35)
    return h
end
function VI.Key(p, c)
    local h = new("Frame", { Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, Parent = p })
    ...
    rf(h, UDim2.new(0,2,0,3), UDim2.new(0,15,0,7), c, 1)
    return h
end
function VI.Close(p, c)
    local h = new("Frame", { Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, Parent = p })
    rf(h, UDim2.new(0,12,0,2.5), UDim2.new(0,1,0,5.7), c, 2, 45)
    rf(h, UDim2.new(0,12,0,2.5), UDim2.new(0,1,0,5.7), c, 2, -45)
    return h
end
function VI.Minimize(p, c)
    local h = new("Frame", { Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, Parent = p })
    rf(h, UDim2.new(0,12,0,2.5), UDim2.new(0,1,0,6.7), c, 2)
    return h
end
function VI.Check(p, c)
    local h = new("Frame", { Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, Parent = p })
    rf(h, UDim2.new(0,7,0,2.5), UDim2.new(0,1,0,7), c, 2, 45)
    rf(h, UDim2.new(0,12,0,2.5), UDim2.new(0,4,0,5), c, 2, -45)
    return h
end
...
local function buildLogo(parent, size)
    ...
    rf(h, UDim2.new(0, 11*sk, 0, 2*sk), UDim2.new(0, 7*sk, 0, 15*sk), Theme.AccentGlow, 1, -25)
    return h
end
...
local function tryClipboard()
    ...
    for _, fn in ipairs(funcs) do
        ...
        if ok and res and tostring(res) ~= "" and tostring(res) ~= "nil" and tostring(res) ~= "Nil" then
            return tostring(res)
        end
    end
    return nil
...
local function showToast(title, desc, kind)
    ...
    local glowStroke = new("UIStroke", { Color = color, Thickness = 1.5, Transparency = 0.7, Parent = toast })
    ...
    tween(toast, 0.45, { Size = UDim2.new(1, 0, 0, 76), BackgroundTransparency = 0 }, Enum.EasingStyle.Back)
...
local function buildKeyUI(onSuccess)
    ...
    local glow = new("UIStroke", { Color = Theme.Accent, Thickness = 2, Transparency = 0.6, Parent = KeyCard })
    task.spawn(function()
        while KeyCard.Parent do
            tween(glow, 2, { Transparency = 0.15 }); task.wait(2)
    ...
    local function updScale()
        ...
        uk.Scale = math.min((vp.X * 0.9) / 400, (vp.Y * 0.9) / 300, 1)
    end
    updScale()
    ...
    tBG(Header, "Sidebar")
    corner(16, Header)
    local hFix = new("Frame", { Size = UDim2.new(1, 0, 0, 16), Position = UDim2.new(0, 0, 1, -16), BackgroundColor3 = Theme.Sidebar, BorderSizePixel = 0, Parent = Header })
    tBG(hFix, "Sidebar")

    local lh = new("Frame", { Size = UDim2.new(0, 40, 0, 40), Position = UDim2.new(0, 20, 0.5, -20), BackgroundTransparency = 1, Parent = Header })
    buildLogo(lh, 40)
    ...
    local ki = new("Frame", { Size = UDim2.new(0, 16, 0, 16), Position = UDim2.new(0, 14, 0.5, -8), BackgroundTransparency = 1, Parent = InputBox })
    ...
    pasteBtn.MouseButton1Click:Connect(function()
        ...
        if clip then
            ...
            showToast("Paste Failed", "Clipboard not accessible", "error")
        end
    end)
...
local function buildMainUI()
    ...
    local function upd()
        local cam = workspace.CurrentCamera; if not cam then return end
        local vp = cam.ViewportSize
        mScale.Scale = math.min((vp.X * 0.9) / 660, (vp.Y * 0.9) / 460, 1)
    end
    upd()
    if workspace.CurrentCamera then
    ...
    task.delay(0.55, function() if entranceScale then entranceScale:Destroy() end end)
    ...
    tBG(Sidebar, "Sidebar")
    corner(18, Sidebar)
    local sbFix = new("Frame", { Size = UDim2.new(0, 18, 1, 0), Position = UDim2.new(1, -18, 0, 0), BackgroundColor3 = Theme.Sidebar, BorderSizePixel = 0, Parent = Sidebar })
    tBG(sbFix, "Sidebar")

    local brand = new("Frame", { Size = UDim2.new(1, 0, 0, 76), BackgroundTransparency = 1, Parent = Sidebar })
    local bl = new("Frame", { Size = UDim2.new(0, 40, 0, 40), Position = UDim2.new(0, 18, 0.5, -20), BackgroundTransparency = 1, Parent = brand })
    buildLogo(bl, 40)
    ...
    local TopBar = new("Frame", { Size = UDim2.new(1, 0, 0, 64), BackgroundTransparency = 1, Parent = Content })
    ...
    local pageSub = new("TextLabel", {
        Size = UDim2.new(0, 380, 0, 14),
        Position = UDim2.new(0, 24, 0, 40),
        BackgroundTransparency = 1,
    ...
    local function winBtn(iconKey, fb, xOff)
        local b = new("TextButton", {
            Size = UDim2.new(0, 28, 0, 28),
            Position = UDim2.new(1, xOff, 0, 18),
            BackgroundColor3 = Theme.Card,
            BorderSizePixel = 0,
            Text = "",
            Parent = TopBar,
    ...
    local minBtn = winBtn("Minimize", "Minimize", -70)
    local closeBtn = winBtn("Close", "Close", -38)
    ...
    local function minimize()
        ...
        local lh = new("Frame", { Size = UDim2.new(0, 32, 0, 32), Position = UDim2.new(0.5, -16, 0.5, -16), BackgroundTransparency = 1, Parent = floatingIcon })
        buildLogo(lh, 32)

        tween(floatingIcon, 0.4, { Size = UDim2.new(0, 56, 0, 56) }, Enum.EasingStyle.Back)
    ...
    closeBtn.MouseButton1Click:Connect(function()
        showConfirm("Close Script", "Are you sure you want to close Hallwayz Hub?", function()
            NOFLY()
            ScreenGui:Destroy()
        end)
    end)
    ...
    new("UIListLayout", { Padding = UDim.new(0, 14), SortOrder = Enum.SortOrder.LayoutOrder, Parent = InfoScroll })
    ...
    new("UIListLayout", { Padding = UDim.new(0, 10), SortOrder = Enum.SortOrder.LayoutOrder, Parent = PlayerList })
    ...
    local function devRow(y, label, value, url)
        ...
        row.MouseEnter:Connect(function() tween(row, 0.12, { BackgroundColor3 = Theme.Border }) end)
        row.MouseLeave:Connect(function() tween(row, 0.12, { BackgroundColor3 = Theme.CardHover }) end)
        row.MouseButton1Click:Connect(function()
            pcall(function() game:GetService("GuiService"):OpenBrowserWindow(url) end)
        end)
    end
    devRow(50, "TikTok", CONFIG.Developer.TikTok, "https://tiktok.com/" .. CONFIG.Developer.TikTok)
    devRow(86, "Discord", CONFIG.Developer.Discord, "https://" .. CONFIG.Developer.Discord)
    devRow(122, "YouTube", CONFIG.Developer.YouTube, "https://youtube.com/" .. CONFIG.Developer.YouTube)
    ...
    new("UIListLayout", { Padding = UDim.new(0, 14), SortOrder = Enum.SortOrder.LayoutOrder, Parent = MainScroll })
    ...
    local function updTog()
        if flyOn then
            ...
            tween(togDot, 0.15, { Position = UDim2.new(1, -28, 0.5, -12), BackgroundColor3 = Color3.fromRGB(255,255,255) })
            fst.Text = "Enabled"
            fst.TextColor3 = Theme.Success
            fs.Color = Theme.Accent
        else
            tween(togBg, 0.15, { BackgroundColor3 = Theme.Sidebar })
            tween(togDot, 0.15, { Position = UDim2.new(0, 4, 0.5, -12), BackgroundColor3 = Theme.TextMuted })
            fst.Text = "Disabled"
    ...
    local function updSlider(input)
        ...
        slDot.Position = UDim2.new(pos, -8, 0.5, -8)
    ...
    new("UIListLayout", { FillDirection = Enum.FillDirection.Horizontal, Padding = UDim.new(0, 8), SortOrder = Enum.SortOrder.LayoutOrder, Parent = quickRow })
    ...
    new("UIGridLayout", {
        CellSize = UDim2.new(0.5, -8, 0, 68),
        CellPadding = UDim.new(0, 12, 0, 12),
        SortOrder = Enum.SortOrder.LayoutOrder,
        Parent = MiscScroll,
    })
    ...
    local function selectPage(id)
        ...
        for _, b in pairs(sideButtons) do
            local act = (b._id == id)
            tween(b, 0.15, { BackgroundColor3 = act and Theme.CardHover or Theme.Sidebar })
            tween(b._label, 0.15, { TextColor3 = act and Theme.Text or Theme.TextMuted })
    ...
    for i, p in ipairs(Pages_) do
        ...
        task.delay(0.4 + i * 0.1, function()
            tween(Btn, 0.45, { Position = UDim2.new(0, 0, 0, 0) }, Enum.EasingStyle.Back)
        end)
...
local success, err = pcall(function()
    buildKeyUI(function()
        buildMainUI()
        task.wait(0.6)
        showToast("Welcome", "Hallwayz Hub loaded successfully", "success")
    end)
end)

if not success then
    warn("[Hallwayz] Error: " .. tostring(err))
end
